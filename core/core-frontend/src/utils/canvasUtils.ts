...
export function initCanvasDataPrepare(dvId, busiFlag, callBack) {
  ...
  method(dvId, busiFlagCustom, attachInfo)
    .then(res => {
      ...
      try {
        const settingContent =
          typeof canvasInfo.watermarkInfo?.settingContent === 'string'
            ? JSON.parse(canvasInfo.watermarkInfo.settingContent)
            : canvasInfo.watermarkInfo?.settingContent || {}
        watermarkInfo = {
          ...canvasInfo.watermarkInfo,
          settingContent: settingContent
        }
      } catch (parseError) {
        console.error('解析水印配置失败:', parseError)
        watermarkInfo = {
          ...canvasInfo.watermarkInfo,
          settingContent: {}
        }
      ...
      // 安全解析组件数据
      let canvasDataResult
      try {
        if (typeof canvasInfo.componentData === 'string') {
          canvasDataResult = JSON.parse(canvasInfo.componentData)
        } else {
          canvasDataResult = canvasInfo.componentData
        }
      } catch (parseError) {
        console.error('解析组件数据失败:', parseError)
        console.error('原始数据:', canvasInfo.componentData)
        throw new Error('画布组件数据格式错误，无法解析')
      }

      // 安全解析画布样式数据
      let canvasStyleResult
      try {
        if (typeof canvasInfo.canvasStyleData === 'string') {
          canvasStyleResult = JSON.parse(canvasInfo.canvasStyleData)
        } else {
          canvasStyleResult = canvasInfo.canvasStyleData
        }
      } catch (parseError) {
        console.error('解析画布样式数据失败:', parseError)
        console.error('原始数据:', canvasInfo.canvasStyleData)
        throw new Error('画布样式数据格式错误，无法解析')
      }
      ...
      const curPreviewGap =
        ...
        canvasStyleResult['dashboard'] &&
        canvasStyleResult['dashboard'].gap === 'yes'
    ...
    .catch(error => {
      ...
      ElMessage.error(error.message || '加载画布数据失败，请刷新重试')
    })
}
...
export async function decompressionPre(params, callBack) {
  let deTemplateData
  try {
    const response = await decompression(params)
    const deTemplateDataTemp = response.data

    if (!deTemplateDataTemp) {
    ...
    try {
      if (typeof deTemplateDataTemp['componentData'] === 'string') {
        sourceComponentData = JSON.parse(deTemplateDataTemp['componentData'])
      } else {
        sourceComponentData = deTemplateDataTemp['componentData']
      }
    } catch (parseError) {
      console.error('解析组件数据失败:', parseError)
      console.error('原始数据:', deTemplateDataTemp['componentData'])
      throw new Error('模板组件数据格式错误，无法解析')
    }

    const appData = deTemplateDataTemp['appData']

    if (Array.isArray(sourceComponentData)) {
      ...
      throw new Error('组件数据格式错误，应该是数组')
    }

    // 安全解析 canvasStyleData
    let sourceCanvasStyle
    try {
      if (typeof deTemplateDataTemp['canvasStyleData'] === 'string') {
        sourceCanvasStyle = JSON.parse(deTemplateDataTemp['canvasStyleData'])
      } else {
        sourceCanvasStyle = deTemplateDataTemp['canvasStyleData']
      }
    } catch (parseError) {
      console.error('解析画布样式数据失败:', parseError)
      console.error('原始数据:', deTemplateDataTemp['canvasStyleData'])
      throw new Error('模板画布样式数据格式错误，无法解析')
    }

    //历史字段适配
    sourceCanvasStyle.component = sourceCanvasStyle.component || {}
    ...
    console.error('模板解压失败:', e)
    ElMessage.error(e.message || '模板数据解析失败，请检查模板文件是否正确')
  }
